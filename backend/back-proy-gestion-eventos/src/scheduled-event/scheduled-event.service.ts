...
/*     async inscribe(email:string, scheduledEventId:number){
 ...
 /*        const scheduledEvent=await this.scheduledEventRepository.findOne(body.name);
        ...
        let newStartDate = new Date(body.startDate)
        let newEndDate = new Date(body.endDate)
        if(newStartDate>(await eventoGlobal).endDate || newEndDate>(await eventoGlobal).endDate){
            return{
                "message":"TALLER O CONFERENCIA NO CREADO, El evento ya ha finalizado o esta ingresando fechas fuere del limite del evento"
            }
        } 
